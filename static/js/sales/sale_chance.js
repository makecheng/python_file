...
// ----------------------------------修改营销机会-----------------start---------------
// 初始化修改营销机会对话框
$('#sales_change_update_dialog').dialog({
    title: '修改营销机会',
    width: 700,
    height: 450,
    iconCls: 'icon-edit',
    ...
    buttons: [{
        ...
        iconCls: 'icon-cancel',
        handler: function () {
            $('#sales_change_update_dialog').dialog('close');
            // 清空对话框
            $('#sales_change_update_dialog input').val('');
            $('#sales_change_update_dialog textarea').val('');
            $('#sales_change_update_dialog select').empty();
        }
    }]
});
// 打开修改营销机会对话框
function open_update_sale_chance_dialog() {
    // 判断且必须选择一条才可以弹出对话框
    ...
    $('#sales_change_update_dialog').dialog('open');
}
...
function get_update_linkName() {
    ...
    // 联系人名称赋值给隐藏域
    $('#update_linkManName_hidden').val($('#update_update_linkman_select').find("option:selected").text());
...
function delect_sale_chance() {
    ...
    $.messager.confirm('系统提示','你确认要删除这'+ids.length+'条信息码?',function (r) {
        if(r){
            // 拼接主键
            var sale_chance_ids = '';
